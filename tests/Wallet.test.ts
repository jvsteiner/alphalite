...
describe('Wallet', () => {
  ...
  describe('identity management', () => {
    ...
    it('should not remove the last identity', async () => {
      ...
      expect(() => wallet.removeIdentity(identity.id)).toThrow('Cannot remove the last identity');
    });
  ...
  describe('serialization', () => {
    ...
    it('should export and import wallet encrypted', async () => {
      ...
      const imported = await Wallet.fromJSON(json, { password: 'test-password-123' });
    ...
    it('should fail to import encrypted wallet with wrong password', async () => {
      ...
      await expect(Wallet.fromJSON(json, { password: 'wrong' })).rejects.toThrow();
    });
  });
